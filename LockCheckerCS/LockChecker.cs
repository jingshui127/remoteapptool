...
namespace LockChecker
{
    public class LockChecker
    {
        ...
        private static extern int RmRegisterResources(uint pSessionHandle, uint nFiles, string[] rgsFilenames, uint nApplications, [In] RM_UNIQUE_PROCESS[] rgApplications, uint nServices,
        ...
        public string CheckLock(string fileName)
        {
            uint handle;
            ...
            try
            {
                uint pnProcInfoNeeded = 0, pnProcInfo = 100, lpdwRebootReasons = RmRebootReasonNone;
                ...
                if (res == 0)
                {
                    ...
                    if (pnProcInfo > 0)
                    {
                        ...
                        {
                            result = result + "File Name :" + resources[0] + Environment.NewLine;
                            result = result + "Application locking the file : " + processInfo[i].strAppName + Environment.NewLine;
                            result = result + "PID of process locking the file: " + processInfo[i].Process.dwProcessId.ToString() + Environment.NewLine;
                        }
                    }
                    else
                    {
